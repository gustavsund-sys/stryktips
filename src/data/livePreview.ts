...
function previewLeague(code: 'PL' | 'ELC', name: string, teams: string[], now: Date): LeagueTable {
  const offset = code === 'PL' ? 100 : 200;
  return { code, name, season: '2026', currentMatchday: 4, updatedAt: now.toISOString(), table: teams.map((team, index) => { const playedGames = 4; const won = Math.max(0, 3 - Math.floor(index / 6)); const draw = index % 3 === 0 ? 1 : 0; const lost = playedGames - won - draw; const goalsFor = Math.max(2, 10 - Math.floor(index / 3)); const goalsAgainst = 2 + Math.floor(index / 4); return { position: index + 1, team: { id: offset + index, name: team, shortName: team, tla: team.slice(0, 3).toUpperCase() }, playedGames, won, draw, lost, points: won * 3 + draw, goalsFor, goalsAgainst, goalDifference: goalsFor - goalsAgainst }; }), previousRound: { matchday: 3, matches: teams.slice(0, Math.min(teams.length, 20)).reduce<NonNullable<LeagueTable['previousRound']>['matches']>((matches, team, index, all) => { if (index % 2) return matches; const opponent = all[index + 1]; if (!opponent) return matches; matches.push({ id: offset * 100 + index, utcDate: new Date(now.getTime() - (7 * 24 - index) * 60 * 60_000).toISOString(), status: 'FINISHED', matchday: 3, homeTeam: { id: offset + index, name: team, shortName: team }, awayTeam: { id: offset + index + 1, name: opponent, shortName: opponent }, score: { home: (index / 2) % 3, away: (index / 2 + 1) % 3 } }); return matches; }, []) } };
}
