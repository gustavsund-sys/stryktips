...
import { getAliasReview, getCurrentRound, getLatestRunStatus } from './services/firebase';
import type { AliasReview, Classification, LatestRunStatus, Round } from './types';
...
export default function App() {
  ...
  const [review, setReview] = useState<AliasReview>(); const [latestRun, setLatestRun] = useState<LatestRunStatus>(); const [error, setError] = useState(''); const [filter, setFilter] = useState<Filter>('all');
  useEffect(() => { Promise.all([getCurrentRound(), getAliasReview(), getLatestRunStatus()]).then(([result, aliasReview, runStatus]) => { setRound(result.round); setDemo(result.demo); setLatestRun(runStatus); if (aliasReview?.status === 'pending' && aliasReview.candidates.length) setReview(aliasReview); }).catch(() => setError('Kupongen kunde inte hämtas. Försök igen senare.')); }, []);
  ...
  return <>
    <header className="hero"><nav><a className="brand" href="./" aria-label="TS-Gubbarnas Dundertips"><img src={logo} alt="TS-Gubbarnas Dundertips"/></a><span className="live"><i/> Uppdaterad</span></nav><div className="eyebrow">Veckans kupong · {swedishDate(round.roundDate)}</div><h1>Tretton rätt,<br/><em>Plättlätt!</em></h1><p className="intro">Experttips från Rekatochklart, Bettingstugan, Understreckat och Tipsmedoss — jämförda match för match.</p><div className="meta"><span><b>13</b> matcher</span><span><b>{round.expertCount}</b> experter</span><span><b>{new Intl.DateTimeFormat('sv-SE', { hour: '2-digit', minute: '2-digit' }).format(new Date(round.updatedAt))}</b> uppdaterad</span></div></header>
    <main>
