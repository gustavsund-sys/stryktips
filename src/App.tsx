...
export default function App() {
  ...
  return <>
    <header className="hero"><nav><a className="brand" href="./" aria-label="TS-Gubbarnas Dundertips"><img src={logo} alt="TS-Gubbarnas Dundertips"/></a><span className="live"><i/> Uppdaterad</span></nav><div className="hero-grid"><div className="hero-copy"><div className="eyebrow">Veckans kupong · {swedishDate(round.roundDate)}</div><h1>Tretton rätt,<br/><em>Plättlätt!</em></h1><p className="intro">{round.officialOnly ? 'Svenska Spels nya kupong är publicerad. Expertanalyserna läggs till när de finns tillgängliga.' : 'Experttips från Rekatochklart, Bettingstugan, Understreckat och Tipsmedoss — jämförda match för match.'}</p><div className="meta"><span><b>13</b> matcher</span><span>{round.officialOnly ? <b>Experttips inväntas</b> : <><b>{round.expertCount}</b> experter</>}</span><span><b>{new Intl.DateTimeFormat('sv-SE', { hour: '2-digit', minute: '2-digit' }).format(new Date(round.updatedAt))}</b> uppdaterad</span></div></div><LiveView live={liveStatus} claim={currentClaim}/></div></header>
    <main>
      ...
      <ExpertSourceStatus sources={round.sources}/>
