...
import { getCurrentRound } from './services/firebase';
import type { Classification, Round } from './types';
...
export default function App() {
  ...
  const [error, setError] = useState(''); const [filter, setFilter] = useState<Filter>('all');
  useEffect(() => { getCurrentRound().then((result) => { setRound(result.round); setDemo(result.demo); }).catch(() => setError('Kupongen kunde inte hämtas. Försök igen senare.')); }, []);
  ...
  return <>
    <header className="hero"><nav><a className="brand" href="/"><span>ST</span>Stryktipset<small>Expertkonsensus</small></a><span className="live"><i/> Uppdaterad</span></nav><div className="eyebrow">Veckans kupong · {swedishDate(round.roundDate)}</div><h1>När experterna<br/><em>tycker lika.</em></h1><p className="intro">Experttips från Rekatochklart och Bettingstugan — jämförda match för match.</p><div className="meta"><span><b>13</b> matcher</span><span><b>{round.expertCount}</b> experter</span><span><b>{new Intl.DateTimeFormat('sv-SE', { hour: '2-digit', minute: '2-digit' }).format(new Date(round.updatedAt))}</b> uppdaterad</span></div></header>
    <main>
      ...
      {demo && <div className="notice">Förhandsvisning med demodata — anslut Firebase för livekupongen.</div>}
