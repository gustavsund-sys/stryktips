...
export interface Match { matchNumber: number; homeTeam: string; awayTeam: string; ballots: Ballot[]; support: Record<Sign, number>; classification: Classification; consensusSign?: Sign; systemTip: Tip; publicDistribution?: Record<Sign, number>; odds?: Record<Sign, number>; expertDeviation?: { sign: Sign; difference: number }; xStats?: XStatsData; }
...
export interface Round { roundDate: string; updatedAt: string; status: 'ok' | 'partial'; officialOnly?: boolean; matches: Match[]; sources: Record<string, SourceStatus>; expertCount: number; systemRows: number; publicDistribution: null; xStatsCoverage?: number; highChaparral?: { tips: Tip[]; rows: number; pivots: number[]; singleRow?: Sign[]; estimatedOdds?: number }; }
