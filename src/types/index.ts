...
export interface Ballot { expertId: string; expert: string; source: 'rekatochklart' | 'bettingstugan' | 'understreckat'; sourceUrl: string; tip: Tip; }
...
export interface Round { roundDate: string; updatedAt: string; status: 'ok' | 'partial'; matches: Match[]; sources: Record<string, SourceStatus>; expertCount: number; systemRows: number; publicDistribution: null; highChaparral?: { tips: Tip[]; rows: number; pivots: number[]; estimatedOdds?: number }; }
