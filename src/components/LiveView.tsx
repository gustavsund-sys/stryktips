...
export function LiveView({ live, claim, roundDate, connectionError = false }: { live?: LiveStatus; claim?: ClaimRound; roundDate: string; connectionError?: boolean }) {
  ...
  const freshness = <div className={`live-freshness ${critical ? 'critical' : stale ? 'stale' : ''}`}>{critical ? 'Liveanslutningen är störd' : stale ? 'Uppdateringen är försenad' : 'Senast kontrollerad'} · {checkedLabel}{live.consecutiveFailures ? ` · ${live.consecutiveFailures} misslyckade försök` : ''}</div>;
  if (!live.started) {
    ...
    return <section className="live-view countdown-view"><header><span className="live-pulse"><Clock3 size={16}/></span><div><small>Nästa omgång</small><strong>Avspark om</strong></div><b>{days ? `${days}d ${hours}h` : `${hours}h ${minutes}m`}</b></header>{freshness}<div className="countdown-matches">{live.matches.map((match) => <div key={match.matchNumber}><span>{match.matchNumber}</span><strong>{match.homeTeam}–{match.awayTeam}</strong><time dateTime={match.matchStart}>{new Intl.DateTimeFormat('sv-SE', { weekday: 'short', hour: '2-digit', minute: '2-digit' }).format(new Date(match.matchStart))}</time></div>)}</div></section>;
  }
