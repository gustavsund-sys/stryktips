...
export function LiveView({ live, claim, roundDate, connectionError = false }: { live?: LiveStatus; claim?: ClaimRound; roundDate: string; connectionError?: boolean }) {
  ...
  const freshness = <div className={`live-freshness ${critical ? 'critical' : stale ? 'stale' : ''}`}>{critical ? 'Offline, nytt försök pågår' : stale ? 'Uppdateringen är försenad' : 'Senast kontrollerad'} · {checkedLabel}{live.consecutiveFailures ? ` · ${live.consecutiveFailures} misslyckade försök` : ''}</div>;
  if (!live.started) {
