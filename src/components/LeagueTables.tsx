...
function TableContent({ league, sourceUrl }: { league: LeagueTable; sourceUrl: string }) {
  const updated = new Intl.DateTimeFormat('sv-SE', { weekday: 'short', day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' }).format(new Date(league.updatedAt));
  return <>
    ...
    <footer><span>Säsong {league.season}/{String(Number(league.season) + 1).slice(-2)}{league.currentMatchday ? ` · omgång ${league.currentMatchday}` : ''}</span><span>Uppdaterad {updated} · <a href={sourceUrl} target="_blank" rel="noreferrer">football-data.org ↗</a></span></footer>
  </>;
}
